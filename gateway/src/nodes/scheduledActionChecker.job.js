 ...
 */
export default function scheduledActiontCheckerJob(config, scheduledActionModel, actionNodeModel, nodeModel, sensorModel) {
  return new ScheduledActionCheckerJob(config, scheduledActionModel, actionNodeModel, nodeModel, sensorModel);
}
scheduledActiontCheckerJob.$inject = ['config', 'scheduledActionModel' , 'actionNodeModel', 'nodeModel', 'sensorModel'];
...
export class ScheduledActionCheckerJob {
   ...
   * @param {SensorModel} sensorModel sensor model
   */
  constructor(config, scheduledActionModel, actionNodeModel, nodeModel, sensorModel) {
    this._config = config;
    ...
    this._logger = log4js.getLogger('ScheduledActionCheckerJob');
    this._mqttClient = mqttClient.connect(`mqtt://127.0.0.1:${this._config.mosca.port}`, {clientId: 'ScheduledActionCheckerJob'});
  ...
  async _initJobs() {
    ...
    scheduledActions.forEach(action => {
      this._scheduler.scheduleJob(action.schedule, this._applyScheduledAction.bind(this, action));
    });
  }
  ...
  async _applyScheduledAction(scheduledAction) {
    const actionNodes = await this._actionNodeModel.ActionNode.findActionNodesByEmitterIdAndType(scheduledAction._id, 'SCHEDULED');
    ...
    for(let i = 0; i < actionNodes.length; i++) {
      const sensor = sensors.find(sensor => sensor._id.toString() === actionNodes[i].targetSensorId);
      const node = nodes.find(node => node._id.toString() === sensor.nodeId);

      await this._mqttClient.publish(`cmnd/${node.name}/${sensor.name}/${actionNodes[i].targetSensorType}/`, actionNodes[i].valueToChangeOn.toString());
    }
  }
}
