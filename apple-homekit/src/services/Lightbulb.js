...
module.exports = function (iface) {
  const {amqp, logger, Service, Characteristic} = iface
  ...
  return function createService_Lightbulb(acc, settings, subtype) {
    ...
    if (settings.topic.setBrightness) {
      acc.getService(subtype)
        ...
        .on('set', (value, callback) => {
          logger.debug(`< hap set ${settings.name} Brightness ${value}`)
          current.bri = value
          publishRGB()
      ...
      if (settings.topic.statusBrightness) {
        mqttSub(settings.topic.statusBrightness, settings.json.statusBrightness, val => {
          ...
          logger.debug(`> hap update ${settings.name} Brightness ${brightness}`)
        ...
        acc.getService(subtype)
          ...
          .on('get', callback => {
            logger.debug(`< hap get ${settings.name} Brightness`)
            /* istanbul ignore next */
            ...
            logger.debug(`> hap re_get ${settings.name} Brightness ${brightness}`)
    ...
    if (settings.topic.setHue) {
      acc.getService(subtype)
        ...
        .on('set', (value, callback) => {
          logger.debug(`< hap set ${settings.name} Hue ${value}`)
          current.hue = value
          publishRGB()
          /* istanbul ignore next */
      ...
      if (settings.topic.statusHue) {
        mqttSub(settings.topic.statusHue, settings.json.statusHue, val => {
          ...
          logger.debug(`> hap update ${settings.name} Hue ${hue}`)
          current.hue = hue
          acc.getService(subtype)
            .updateCharacteristic(Characteristic.Hue, hue)
        })
    ...
    if (settings.topic.setSaturation) {
      acc.getService(subtype)
        ...
        .on('set', (value, callback) => {
          logger.debug(`< hap set ${settings.name} Saturation ${value}`)
          current.sat = value
          publishRGB()
          /* istanbul ignore next */
      ...
      if (settings.topic.statusSaturation) {
        mqttSub(settings.topic.statusSaturation, settings.json.statusSaturation, val => {
          ...
          logger.debug(`> hap update ${settings.name} Saturation ${sat}`)
          current.sat = sat
          acc.getService(subtype)
            .updateCharacteristic(Characteristic.Saturation, sat)
        })
        acc.getService(subtype)
          ...
          .on('get', callback => {
            logger.debug(`< hap get ${settings.name} Saturation`)
            /* istanbul ignore next */
            ...
            logger.debug(`> hap re_get ${settings.name} Saturation ${saturation}`)
    ...
    if (settings.topic.setColorTemperature) {
      acc.getService(subtype)
        ...
        .on('set', (value, callback) => {
          logger.debug(`< hap set ${settings.name} ColorTemperature ${value}`)
          const sat = value
      ...
      if (settings.topic.statusColorTemperature) {
        mqttSub(settings.topic.statusColorTemperature, settings.json.statusColorTemperature, val => {
          const sat = val
          logger.debug(`> hap update ${settings.name} ColorTemperature ${sat}`)
          acc.getService(subtype)
        ...
        acc.getService(subtype)
          ...
          .on('get', callback => {
            logger.debug(`< hap get ${settings.name} ColorTemperature`)
            /* istanbul ignore next */
            const saturation = mqttStatus(settings.topic.statusColorTemperature, settings.json.statusColorTemperature)
            logger.debug(`> hap re_get ${settings.name} ColorTemperature ${saturation}`)
            callback(null, saturation)
          })
      }
    }
